...
export default function AdminPage() {
  ...
  // single source of truth that gets submitted; uploaded URLs just get appended to it.
  ...
  // --- New: uploads selected files to Cloudinary via our backend route,
  // then merges the returned hosted URLs into the given comma-separated
  // "images" string state (same format the existing URL field already uses).
  async function uploadImagesToServer(files, currentImagesValue, setImagesValue, setUploadingFlag, setFilesState) {
    ...
    try {
      const { data } = await API.post(`${PRODUCTS_API}/upload-images`, formData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
          "Content-Type": "multipart/form-data",
        },
      ...
      const existing = currentImagesValue
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
      setImagesValue([...existing, ...newUrls].join(", "));
      setFilesState([]);
    } catch (err) {
  ...
  return (
    <div style={{ padding: 12 }}>
      ...
      <form onSubmit={handleAdd} style={{ margin: "16px 0", display: "grid", gap: 8, maxWidth: 420 }}>
        ...
        {/* New: direct image upload — sits alongside the URL field above, does not replace it */}
        <div style={{ display: "grid", gap: 6, border: "1px dashed #bbb", borderRadius: 6, padding: 8 }}>
          <label style={{ fontSize: 13, color: "#555" }}>Or upload image files directly</label>
          <input
            type="file"
            accept="image/*"
            multiple
          ...
          <button
            ...
            onClick={() =>
              uploadImagesToServer(uploadFiles, images, setImages, setUploading, setUploadFiles)
            }
          >
            ...
            <li key={p._id} style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12 }}>
              {editingId === p._id ? (
                <div style={{ display: "grid", gap: 8 }}>
                  ...
                  {/* New: direct image upload for edit form — additive, same pattern as Add form */}
                  <div style={{ display: "grid", gap: 6, border: "1px dashed #bbb", borderRadius: 6, padding: 8 }}>
                    <label style={{ fontSize: 13, color: "#555" }}>Or upload image files directly</label>
                    <input
                      type="file"
                    ...
                    <button
                      type="button"
                      disabled={editUploading || editUploadFiles.length === 0}
                      onClick={() =>
